...
export class Project_List extends Component {
  ...
  getProjects = () => {
    if (this.props.user.user_id) {
      axios
        .get(`/api/getProjects/${this.props.user.user_id}`)
  ...
  render() {
    ...
    if (this.props.searchInput) {
      filteredProjects = projects.filter(
        p =>
          p.project_name.includes(this.props.searchInput) ||
          p.project_description.includes(this.props.searchInput)
      );
    } else {
    ...
    return (
      <div className={this.props.toggleSideBar ? "projects" : "projects open"}>
        <div className="idea">
          <FaPlus
            onClick={() =>
              this.props.history.push(`/newProject/${this.props.user.user_id}`)
            }
            size={80}
            style={{ margin: "auto", color: "yellowgreen" }}
          ></FaPlus>
        </div>
        {filteredProjects.map(idea => (
          <div
            className="idea"
            onClick={() =>
              this.props.history.push(`/single/${idea.project_id}`)
            }
          >
            <div key={idea.project_id} className="project-name">
              {" "}
              {idea.project_name}{" "}
            </div>
            <div className="lower-box">
              ...
              <div className="project-deadline">
                {" "}
                Deadline: {idea.deadline.slice(0, 10)}{" "}
              </div>
              <div className="members-line">
                <div>
                  ...
                  <IoIosPeople
                    size={50}
                    style={{ margin: "10px", color: "#33adff" }}
                  ></IoIosPeople>{" "}
                </div>
                <div>
                  {" "}
                  {idea.teammates.length <= 4 ? (
                    idea.teammates.map(u => (
                      <img
                        src={u.profile_image}
                        width="40"
                    ...
                    <div style={{ display: "flex" }}>
                      {idea.teammates.slice(0, 4).map(p => (
                        <img
                          src={p.profile_image}
                          width="40"
                          style={{ height: "40px", borderRadius: "50%" }}
