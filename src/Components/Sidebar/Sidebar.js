...
const logoutStyle = {
  content: {
    ...
    alignItems: "center",
    backgroundColor: "rgb(168, 134, 255)",
  },
};
...
export class Sidebar extends Component {
  ...
  render() {
    ...
    return (
      <div
        className={this.props.toggleSideBar ? "show_side_bar" : "hide_side_bar"}
      >
        <Modal isOpen={isModelOpen} style={logoutStyle}>
          <p
            style={{
              display: "flex",
        ...
        <div
          onClick={() =>
            this.props.history.push(
              `/personal_stats/${this.props.user.user_id}`
            )
          }
          className="sidebar_links"
        >
        ...
        <div id="logout_container">
          {" "}
          <AiOutlineLogout id="logout" onClick={this.openModel}>
            {" "}
          </AiOutlineLogout>
        </div>
      </div>
...
export default withRouter(
  connect(mapStateToProps, { sidebarToggle, userLoggedIn, userLoggedOut })(
    Sidebar
  )
);
