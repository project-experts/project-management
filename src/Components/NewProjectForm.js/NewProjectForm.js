...
export class NewProjectForm extends Component {
  ...
  addUserToTeam = () => {
    ...
    console.log(user);
    this.setState({
      assignedMembers: [...this.state.assignedMembers, user[0]]
    });
  };
  ...
  render() {
    ...
    if (searchBy.length >= 1) {
      filteredUsers = users.filter(
        u => u.first_name.includes(searchBy) || u.last_name.includes(searchBy)
      );
    } else {
      filteredUsers = users;
    }
    ...
    return (
      <div
        className={
          this.props.toggleSideBar ? "new-project" : "new-project open"
        }
      >
        <Modal
          isOpen={modalIsOpen}
        ...
        >
          <div style={{ display: "flex", justifyContent: "center" }}>
            Are you sure you want to add {firstName} {lastName} to this Project
            ?
          </div>
          <div
            style={{
            ...
            <div>
              {assignedMembers.length > 0 ? (
                assignedMembers.map(u => (
                  <div className="assigned-members" key={u.user_id}>
                    {" "}
                    <img
                      src={u.profile_image}
                      width="40"
