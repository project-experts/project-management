module.exports = {
  ...
  // Count all different tasks by single user
  countToDoTasks: (req, res) => {
    const db = req.app.get("db");
    const { user_id } = req.params;
    db.tasks
      .get_allTasks_singleUser_todo_count(user_id)
      .then(data => console.log("data :", data) || res.status(200).send(data));
  },

  countInProgressTasks: (req, res) => {
    const db = req.app.get("db");
    const { user_id } = req.params;
    db.tasks
      .get_allTasks_singleUser_inProgress_count(user_id)
      .then(data => res.status(200).send(data));
  },

  countReviewTasks: (req, res) => {
    const db = req.app.get("db");
    const { user_id } = req.params;
    db.tasks
      .get_allTasks_singleUser_review_count(user_id)
      .then(data => res.status(200).send(data));
  },
  countDoneTasks: (req, res) => {
    const db = req.app.get("db");
    const { user_id } = req.params;
    db.tasks
      .get_allTasks_singleUser_done_count(user_id)
      .then(data => res.status(200).send(data));
  },
  //Delete task
  deleteTask: (req, res) => {
  ...
  // Get tasks single project & all teammates
  ...
  getAllTasksSingleProject: async (req, res) => {
    ...
    await db.tasks
      .get_all_tasks_single_project(project_id)
      .then(response => res.status(200).send(response))
